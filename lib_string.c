...
static int str_rtrim(lua_State *L)
{
    ...
    ch_len = s_len = 0;
    s = strndupa(luaL_checklstring(L, 1, &s_len), s_len);
    ch = luaL_checklstring(L, 2, &ch_len);

    if( ch_len <= 1 ) {
	lua_pushstring(L, strrtrim(s, ch_len ? *ch : ' '));
    } else {
	lua_pushstring(L, strrtrim_multi(s, ch, ch_len));
    }
    return 1;
}
...
static int str_ltrim(lua_State *L)
{
    ...
    if( ch_len <= 1 ) {
	lua_pushstring(L, strltrim(s, ch_len ? *ch : ' '));
    } else {
	lua_pushstring(L, strltrim_multi(s, ch, ch_len));
    }
    return 1;
}
