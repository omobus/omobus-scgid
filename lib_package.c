...
/* LUA_PATH and LUA_CPATH are the names of the environment variables that Lua 
   check to set its paths. */
#if !defined(LUA_PATH)
...
#endif
/* LUA_IGMARK is a mark to ignore all before it when building the luaopen_ function name */
#if !defined (LUA_IGMARK)
# define LUA_IGMARK		"-"
#endif
/*  LUA_CSUBSEP is the character that replaces dots in submodule names when searching for a C loader */
#if !defined(LUA_CSUBSEP)
# define LUA_CSUBSEP		LUA_DIRSEP
#endif
/* LUA_LSUBSEP is the character that replaces dots in submodule names when searching for a Lua loader */
...
static void *ll_load(lua_State *L, const char *path, int seeglb) {
    void *lib = NULL;
    if( (lib = dlopen(path, RTLD_NOW | (seeglb ? RTLD_GLOBAL : RTLD_LOCAL))) == NULL ) {
...
static lua_CFunction ll_sym(lua_State *L, void *lib, const char *sym) {
    void *func;
    if( (func = dlsym(lib, sym)) == NULL ) {
	lua_pushstring(L, dlerror());
    }
    return (lua_CFunction) func;
}
...
static int ll_loadlib(lua_State *L) {
	...
	lua_pushnil(L);
	lua_insert(L, -2);
	lua_pushstring(L, (stat == ERRLIB) ?  "open" : "init");
...
/* *** omobus-scgid: begin *** */
static 
void setpath(lua_State *L, const char *fieldname, const char *def) 
{
