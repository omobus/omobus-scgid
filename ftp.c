...
int ftp_clear_dir(ftp_ctx_t ctx) 
{
    int rc = OMOBUS_OK, len = 0;
    char *buf = NULL, *fn = NULL, *tmp = NULL;

    if( (rc = ftp_nlst_mem(ctx, &buf, &len)) != OMOBUS_OK ) {
	return rc;
    }
    if( strstr(buf, OMOBUS_FF_UNLOCKED "\n") != NULL ) {
	rc = ftp_dele(ctx, OMOBUS_FF_UNLOCKED);
    }
    fn = buf;
...
static
int sync_stor_cb(void *cookie, const char *name, const char *fn, struct stat *st)
{
    return S_ISREG(st->st_mode) ? ftp_stor_f_safe((ftp_ctx_t *)cookie, name, fn) : OMOBUS_OK;
}

static
int slist_set(dynarray_t da, const char *name)
{
    char **ptr;
    if( (ptr = (char **)da_push(da)) == NULL ) {
	allocate_memory_error(strerror(errno));
	return OMOBUS_ERR;
    }
    *ptr = strdup(name);
    return *ptr == NULL ? OMOBUS_ERR : OMOBUS_OK;
}

static
int slist_set_cb(void *cookie, const char *name, const char *fn, struct stat *st)
{
    return S_ISREG(st->st_mode) ? slist_set(cookie, name) : OMOBUS_OK;
}

static
void slist_cleanup_cb(void *cookie, void *ptr)
{
    chk_free(*((char**)ptr));
}

/* *** LOCAL -> FTP sync (uploads all local files) */
static
int ftp_push_dir_ALL(ftp_ctx_t p, const char *path, const char *ftp_path)
{
    return ftp_cwd(p, ftp_path) == OMOBUS_OK && ftp_clear_dir(p) == OMOBUS_OK && 
	lsdir(path, p, NULL, sync_stor_cb) == OMOBUS_OK ? ftp_unlockdir(p) : OMOBUS_ERR;
}

/* *** LOCAL -> FTP sync (uploads only differences) */
static
int ftp_push_dir_DIFF(ftp_ctx_t p, const char *path, const char *ftp_path)
{
    int rc = OMOBUS_OK, qty = 0, len = 0;
    short unlock = 0;
    char *buf = NULL, *tmp = NULL, *fn = NULL;
    size_t i = 0, j = 0, size[2];
    dynarray_t ar[2];
    hashtable_t idx[2];
    ...
    if( (ar[0] = da_create(sizeof(char**), 0, 1024)) == NULL ||
	(ar[1] = da_create(sizeof(char**), 0, 1024)) == NULL ) {
	allocate_memory_error(strerror(errno));
	rc = OMOBUS_ERR;
    } else if( lsdir(path, ar[0], NULL, slist_set_cb) == OMOBUS_ERR ) {
	logmsg_e(JPREFIX "failed to list %s: %s", path, strerror(errno));
	rc = OMOBUS_ERR;
    } else if( 	(rc = ftp_cwd(p, ftp_path)) == OMOBUS_OK && 
		(rc = ftp_nlst_mem(p, &buf, &len)) == OMOBUS_OK ) {
	/* parse FTP file list */
	fn = buf;
	if( buf != NULL && len > 0 && strstr(buf, OMOBUS_FF_UNLOCKED "\n") != NULL ) {
	    unlock = 1;
	}
	...
	tmp = NULL;
	if( rc == OMOBUS_OK ) {
	    /* synchronize the local and FTP folders */
	    for( i = 0; i < 2; i++ ) {
		size[i] = da_size(ar[i]);
		idx[i] = ht_create(size[i]);
	    }
	    if( (size[0] > 0 && idx[0] == NULL) || (size[1] > 0 && idx[1] == NULL) ) {
		allocate_memory_error(strerror(errno));
		rc = OMOBUS_ERR;
	    } else {
		for( i = 0; i < 2; i++ ) {
		    for( j = 0; j < size[i]; j++ ) {
			ht_set(idx[i], *((char**)da_get(ar[i], j)), NULL);
		    }
		}
		for( i = 0; rc == OMOBUS_OK && i < size[1]; i++ ) {
		    fn = *((char**)da_get(ar[1], i));
		    if( !ht_exist(idx[0], fn) ) {
			if( unlock ) {
			    rc = ftp_lockdir(p);
			    unlock = 0;
			}
			if( rc == OMOBUS_OK && (rc = ftp_dele(p, fn)) == OMOBUS_OK ) {
			    qty++;
			}
		    }
		}
		for( i = 0; rc == OMOBUS_OK && i < size[0]; i++ ) {
		    fn = *((char**)da_get(ar[0], i));
		    if( !ht_exist(idx[1], fn) ) {
			if( unlock ) {
			    rc = ftp_lockdir(p);
			    unlock = 0;
			}
			if( rc == OMOBUS_OK ) {
			    if( asprintf(&tmp, "%s%s", path, fn) == -1 || tmp == NULL ) {
				allocate_memory_error(strerror(errno));
				rc = OMOBUS_ERR;
			    } else if( (rc = ftp_stor_f_safe(p, fn, tmp)) == OMOBUS_OK ) {
				qty++;
			    }
			    chk_free(tmp);
			}
	...
	/* unlock ftp folder*/
	rc = rc == OMOBUS_OK && qty > 0 ? ftp_unlockdir(p) : rc;
    }

    /* cleanup */
    for( i = 0; i < 2; i++ ) {
	ht_destroy(idx[i]);
	da_for_each(ar[i], NULL, slist_cleanup_cb);
	da_destroy(ar[i]);
    }
    chk_free(buf);
...
/* *** FTP -> LOCAL sync (downloads only differences) */
static
int ftp_pull_dir_DIFF(ftp_ctx_t p, const char *ftp_path, const char *path, void *cookie, ftp_pull_cb cb)
{
    int rc = OMOBUS_OK, len = 0;
    char *buf = NULL, *tmp = NULL, *fn = NULL;
    size_t i = 0, j = 0, size[2];
	...
	fn = buf;
	if( !(buf != NULL && len > 0 && strstr(buf, OMOBUS_FF_UNLOCKED "\n") != NULL) ) {
	    logmsg_e(JPREFIX "%s@%s:%u is locked.", ((ftp_ctx *) p)->user, 
		((ftp_ctx *) p)->host, ((ftp_ctx *) p)->port);
	    rc = OMOBUS_ERR;
	}
	while( rc == OMOBUS_OK && *fn != '\0' && len > 0 && (tmp = strchr(fn, '\n')) != NULL ) {
	    *tmp = '\0'; tmp++;
	...
	tmp = NULL;
	if( rc == OMOBUS_OK ) {
	    /* synchronize the FTP and local folders */
	    for( i = 0; i < 2; i++ ) {
		size[i] = da_size(ar[i]);
		idx[i] = ht_create(size[i]);
	    }
		...
		for( i = 0; rc == OMOBUS_OK && i < size[1]; i++ ) {
		    fn = *((char**)da_get(ar[1], i));
		    if( cb != NULL ) {
			cb(cookie, fn, NULL, /*complete:*/ 0 /* no */, i + 1, size[1]);
		    }
		    if( !ht_exist(idx[0], fn) ) {
			if( asprintf(&tmp, "%s%s", path, fn) == -1 || tmp == NULL ) {
			    allocate_memory_error(strerror(errno));
			    rc = OMOBUS_ERR;
			} else if( (rc = ftp_retr_f(p, fn, tmp)) == OMOBUS_OK ) {
			    if( cb != NULL ) {
				cb(cookie, fn, tmp, /*complete:*/ 1 /* yes */, i + 1, size[1]);
			    }
			}
			chk_free(tmp);
		    }
		...
		for( i = 0; rc == OMOBUS_OK && i < size[0]; i++ ) {
			...
			if( asprintf(&tmp, "%s%s", path, fn) == -1 || tmp == NULL ) {
			    allocate_memory_error(strerror(errno));
			    rc = OMOBUS_ERR;
			} else if( unlink(tmp) == -1 ) {
			    logmsg_e(JPREFIX "failed to unlink %s: %s", 
				tmp, strerror(errno));
			    rc = OMOBUS_ERR;
			} else {
#ifdef FTP_TRACE
			    if( ((ftp_ctx *) p)->log != NULL ) {
				fprintf(((ftp_ctx *) p)->log, "*: unlink %s\n", tmp);
			    }
#endif //FTP_TRACE
			}
			chk_free(tmp);
...
{
    return diff ? ftp_push_dir_DIFF(p, path, ftp_path) : ftp_push_dir_ALL(p, path, ftp_path);
}

int ftp_pull_dir(ftp_ctx_t p, const char *ftp_path, const char *path, void *cookie, ftp_pull_cb cb)
{
    return ftp_pull_dir_DIFF(p, ftp_path, path, cookie, cb);
}

/* *** move files from LOCAL -> FTP path */
int ftp_move_dir(ftp_ctx_t p, const char *path, const char *ftp_path, void *cookie, ftp_move_cb cb)
{
    int rc = OMOBUS_OK, x = OMOBUS_ERR;
    char *tmp = NULL, *fn = NULL;
    size_t i = 0, size = 0;
    dynarray_t ar = NULL;

    if( (ar = da_create(sizeof(char**), 0, 1024)) == NULL ) {
	allocate_memory_error(strerror(errno));
	rc = OMOBUS_ERR;
    } else if( lsdir(path, ar, NULL, slist_set_cb) == OMOBUS_ERR ) {
	logmsg_e(JPREFIX "failed to list %s: %s", path, strerror(errno));
	rc = OMOBUS_ERR;
    } else if( (size = da_size(ar)) > 0 && (rc = ftp_cwd(p, ftp_path)) == OMOBUS_OK ) {
	for( i = 0; rc == OMOBUS_OK && i < size; i++ ) {
	    fn = *((char**)da_get(ar, i));
	    if( cb != NULL ) {
		cb(cookie, fn, /*complete:*/ 0 /* no */, i + 1, size);
	    }
	    if( asprintf(&tmp, "%s%s", path, fn) == -1 || tmp == NULL ) {
		allocate_memory_error(strerror(errno));
		rc = OMOBUS_ERR;
	    } else if( (x = ftp_size(p, fn)) >= 0 ) {
		logmsg_e(JPREFIX "%s@%s:%u stored file %s (size: %d bytes) already exist.", ((ftp_ctx *) p)->user,
		    ((ftp_ctx *) p)->host, ((ftp_ctx *) p)->port, fn, x);
	    } else if( (rc = ftp_stor_f_safe(p, fn, tmp)) == OMOBUS_OK ) {
		if( cb != NULL ) {
		    cb(cookie, fn, /*complete:*/ 1 /* yes */, i + 1, size);
		}
		if( unlink(tmp) == -1 ) {
		    logmsg_w(JPREFIX "failed to unlink %s: %s", 
			tmp, strerror(errno));
		    //rc = OMOBUS_ERR;
		} else {
#ifdef FTP_TRACE
    ...
    da_for_each(ar, NULL, slist_cleanup_cb);
    da_destroy(ar);
