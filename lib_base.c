...
/* Removed unsafe functions: print, loadfile, dofile, loadstring.
 * Added functions: iif
 */
...
static int luaB_iif(lua_State *L)
{
    int narg = !lua_isnil(L, 1) && luaL_checkboolean(L, 1) ? 2 : 3;
    if( lua_isnone(L, narg) ) {
	luaL_argerror(L, narg, "unexpected parameter");
    } else {
